...
public class BufferPool {
    ...
    private Map<PageId, Page> m_cache;
    private final int m_maxNumPages;
    ...
    public BufferPool(int numPages) {
        ...
        m_cache = new ConcurrentHashMap<PageId, Page>();
    ...
    public Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        if (perm == Permissions.READ_ONLY) {
            Type = Lock.SHARED_LOCK;
        }
        else Type = Lock.EXCLUSIVE_LOCK;
        ...
        while (!lockType) {
            ...
            lockType = lockManager.acquiresLock(tid, pid, Type);
        }
    ...
    public void transactionComplete(TransactionId tid, boolean commit)
        ...
        if (commit) {
            ...
            for (PageId pid : m_cache.keySet()) {
                Page page = m_cache.get(pid);
                if (page.isDirty() == tid) {
                    //把这些都写入磁盘
                    DbFile dbFile = Database.getCatalog().getDatabaseFile(pid.getTableId());
                    Page pageToRevert = dbFile.readPage(pid);
                    m_cache.put(pid, pageToRevert);
                }
            }
        }
        for (PageId pid : m_cache.keySet()) {
    ...
    public void deleteTuple(TransactionId tid, Tuple t)
        ...
        // some code goes here
        // not necessary for lab1
        //调用file进行删除，然后把返回的一个list全部标记进入脏页
    ...
    public synchronized void flushPages(TransactionId tid) throws IOException {
        ...
        // not necessary for lab1|lab2
        if(m_cache.size() != 0) {
            Iterator<PageId> it = m_cache.keySet().iterator();
            while (it.hasNext()) {
                PageId pid = it.next();
                if (holdsLock(tid, pid)) {
                    flushPage(pid);
    ...
    private synchronized void evictPage() throws DbException {
        // some code goes here
        // not necessary for lab1
        for (Page page:m_cache.values()) {
            // 实现NO STEAL策略，如果还是占用的时候就不能flush
